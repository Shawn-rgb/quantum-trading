...
def check_explosion_setup(symbol):
    ...
    try:
        ...
        # 🌟 核心 Bug 修复：看【昨天】彻底走完的 K 线 (-2)，而不是今天刚开盘 1 小时的 K 线 (-1)
        yesterday_vol = df["volume"][-2]
        avg_vol = df["avg_volume"][-3]  # 对比昨天之前 30 天的均量

        vol_multiplier = yesterday_vol / avg_vol if avg_vol > 0 else 0
        ...
        # 同样看昨天收盘时的布林带状态
        recent_min_width = df["bb_width"][-31:-1].min()
        yesterday_width = df["bb_width"][-2]
        is_squeezed = yesterday_width <= (recent_min_width * 1.5)

        # 👑 触发核心报警逻辑：
        if vol_multiplier > 2.0 and is_squeezed:
            return {
                "symbol": symbol,
                "vol_spike": round(vol_multiplier, 2),
                "close_price": df["close"][-2] # 昨天的收盘价
            }
        return None
...
if __name__ == "__main__":
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        results = executor.map(check_explosion_setup, target_symbols)
